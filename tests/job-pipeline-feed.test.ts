...
describe('job feed production SQL', () => {
  ...
  it('AC-02 captures changes, replays stable IDs, and records deletion without private notes', async () => {
    ...
    await db.query("update job_applications set stage='phone_screen' where id=$1", [id])
    await db.query("update job_applications set follow_up_date=current_date-1 where id=$1", [id])
    await db.query("update job_applications set notes='PRIVATE' where id=$1", [id])
    await db.query('delete from job_applications where id=$1', [id])
    const feed = await queryFeed(start.next_cursor)
    assert.deepEqual(feed.changes.map(change => change.operation), ['INSERT', 'UPDATE', 'UPDATE', 'DELETE'])
    assert.ok(feed.changes.every(change => change.application_id === id))
    assert.equal(feed.changes[1].application.stage, 'phone_screen')
    assert.equal(feed.changes[1].application.follow_up_date, null)
    assert.equal(feed.changes[2].application.stage, 'phone_screen')
    const expectedDate = await db.query<{ date: string }>('select (current_date-1)::text as date')
    assert.equal(feed.changes[2].application.follow_up_date, expectedDate.rows[0].date)
  ...
  it('AC-02/03 orders event sequences numerically and due work by date then identity', async () => {
    const start = await queryFeed()
    await db.exec('begin')
    try {
      await db.exec("insert into job_applications(company,role,follow_up_date) select 'Ordered ' || n, 'Role', current_date-(n%3) from generate_series(1,12) n")
      const feed = await queryFeed(start.next_cursor)
