...
**When you lead with a subset, the first entry in the \`follow_up_suggestions\` array MUST offer ALL remaining unseen items** — e.g. "What are Sunny's other 5 projects?" or "Show all of Sunny's projects" — phrased third-person about the candidate. This is the reader's "show more"; without it they have no path to the rest, which would turn progressive disclosure into omission. Only after that offer may you add deeper-dive follow-ups. This rule applies on every response where the full set has not yet been shown — not just the first one. If a \`shown_projects\` context is present and there are still unseen projects, the remainder offer MUST appear again.
