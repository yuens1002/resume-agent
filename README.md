...
**The listing is authored-only by default.** This surface is crawled — it's server-rendered into a sitemap — and a crawler arriving with no query params is the common case, not the edge one. It should get the reasoning trail the endpoint's own `note` advertises, not sync warnings that read as thin content. `?authored=0` returns machine entries; `?authored=all` returns both.

The flag comes from an **allowlist** of authored `metadata.source` values (`mcp`, the private MCP `capture_thought` path), not a denylist of machine ones — so a machine writer added later is `authored: false` by default instead of silently passing as authored. Filtering happens *before* `limit`, so `?limit=25` returns 25 authored notes rather than whatever survives filtering a mixed page.

The authored default applies **only within the authored layer** (`observation`/`idea`/`task`). An explicit `?type=` outside it — `reference`, `review_needed`, `notification` — returns both classes, because those are machine-written by definition and filtering them to authored-only would empty them rather than filter them, silently breaking the `?type=reference` escape hatch documented above. An explicit `?authored=` always wins over that inference, so every combination stays reachable.

The applied filter is always echoed in the response envelope, which doubles as a **capability probe**: the key is absent on any deployment predating this field, so a client can tell "the server applied my filter" from "the server ignored an unknown param" — otherwise indistinguishable, since an ignored param still returns `200` with plausible JSON.
