...
- Every route (including `/mcp`) is rate-limited to 30 req/min per IP — bypassed for requests carrying either a valid `Authorization: Bearer <API_KEY>` header or a valid `x-brain-key` header (the MCP server's own owner credential)
