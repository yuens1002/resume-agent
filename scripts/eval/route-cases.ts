...
export const ROUTE_CASES: RouteCase[] = [
  ...
  { id: 'legacy-check-fit', expected: 'narrate', source: 'eval-legacy', question: 'Can you check if Sunny would be a good fit for this role? Staff Engineer, Platform team — deep experience with distributed systems and Kubernetes required, plus mentoring junior engineers across a 20-person org.', note: 'Spec pivot 2026-07-10: "can you check IF X fits" is a polite fit question, not a procedure invocation — judge-loop arbitration (sonnet flagged the whole family). Was open_match_tool under #182.' },
  { id: 'legacy-would-this-match', expected: 'narrate', source: 'eval-legacy', question: 'Would this position be a good match for Sunny\'s background? Frontend Lead at a Series B startup, React/TypeScript heavy, need someone who can set technical direction and hire the first 3 engineers.', note: 'Spec pivot 2026-07-10: a question ABOUT fit (even with a JD) narrates; only an action request opens the tool. Was open_match_tool under the #182-era spec.' },
  ...
  { id: 'legacy-prose-fit-fintech', expected: 'narrate', source: 'eval-legacy', question: 'Is Sunny a fit for a Senior Backend Engineer role at a fintech startup needing strong Postgres and API design skills?', note: 'Spec pivot 2026-07-10: fit QUESTION → narrate + follow-up chip. Was open_match_tool under #182.' },
  { id: 'legacy-prose-fit-devops', expected: 'narrate', source: 'eval-legacy', question: 'Any chance Sunny fits a DevOps Lead role requiring Kubernetes and Terraform experience?', note: 'Spec pivot 2026-07-10: fit QUESTION → narrate + follow-up chip. Was open_match_tool under #182.' },
  { id: 'legacy-fit-verb-no-role', expected: 'narrate', source: 'eval-legacy', question: 'Would Sunny be a good hire?', note: 'Fit verb with zero role-identifying signal.' },
  ...
  { id: 'fit-stack-up-ai-lead', expected: 'narrate', source: 'synthetic', question: "We're hiring a Lead AI Engineer (RAG, evals, agent frameworks) — how does Sunny stack up?", note: '"How does X stack up?" is a question about fit, not an action request.' },
  ...
  { id: 'fit-right-for-em', expected: 'narrate', source: 'synthetic', question: 'Is Sunny right for an Engineering Manager role leading a team of 6?', note: 'Owner-decided 2026-07-10 (#195): fit question → narrate. First judge-loop arbitration — sonnet flagged, owner sided with sonnet and pivoted the spec.' },
  { id: 'fit-meet-bar-distsys', expected: 'narrate', source: 'synthetic', question: 'Does Sunny meet the bar for this posting? Senior Software Engineer — distributed systems, Go or Rust, 7+ years.', note: '"Does X meet the bar?" is a question about fit, not an action request.' },
  { id: 'fit-founding-engineer', expected: 'narrate', source: 'synthetic', question: 'How would Sunny do as a founding engineer at an AI startup?', note: 'Owner-decided 2026-07-10 (#195): predictive "how would X do" question → narrate. Judge-loop arbitration.' },
  ...
  { id: 'fit-suits-remote-react', expected: 'narrate', source: 'synthetic', question: 'Evaluate whether Sunny suits a remote Senior React role.', note: '"Evaluate WHETHER…" embeds a yes/no verdict question — prose answers it. Contrast "screen/assess X against…", which requests the matching procedure and opens the tool.' },
  ...
  { id: 'boundary-could-handle', expected: 'narrate', source: 'synthetic', question: 'Could Sunny handle a senior backend role?', note: 'Capability verb ("handle"), not a fit-check ask — the spec requires an explicit check/evaluate/match/tailor ask.' },
  { id: 'boundary-suited-startups', expected: 'narrate', source: 'synthetic', question: 'Is Sunny suited to early-stage startup work?', note: 'No specific role — environment preference, not a fit-check.' },
  { id: 'boundary-ts-strong-enough', expected: 'narrate', source: 'synthetic', question: "Is Sunny's TypeScript strong enough for a senior position?", note: 'Skill-depth question; "a senior position" is generic, not a specific role.' },
  { id: 'boundary-do-well-startup', expected: 'narrate', source: 'synthetic', question: 'Would Sunny do well in a startup environment?' },
  ...
  { id: 'boundary-why-hire', expected: 'narrate', source: 'synthetic', question: 'Why should we hire Sunny?', note: 'Fit-adjacent but zero role signal — a narrated pitch is the right response.' },
  ...
  { id: 'obs-first-person-fit', expected: 'narrate', source: 'observed', question: 'am I a good fit for a senior frontend role?', note: 'Owner-decided 2026-07-10 (#195): read first person literally; the narrated answer can point at the fit-check tool.' },
  ...
  { id: 'obs-mangled-encoding-jd', expected: 'narrate', source: 'observed', question: 'Can you check if Sunny would be a good fit for this role? Staff Engineer, Platform team � deep experience with distributed systems and Kubernetes required.', note: 'Mangled encoding (replacement char) preserved verbatim — robustness case. Label follows legacy-check-fit (same phrasing family, spec pivot 2026-07-10).' },
  ...
  { id: 'multi-intent-bg-plus-fit', expected: 'narrate', source: 'synthetic', question: "Tell me about Sunny's background, and also — is he a fit for this? Senior Data Engineer, Spark + Airflow, remote.", note: 'Spec pivot 2026-07-10: "is he a fit for this?" is a question — narrate both halves; the follow-up chip offers the tool.' },
  { id: 'bare-jd-no-ask', expected: 'narrate', source: 'synthetic', question: 'Senior Platform Engineer (Hybrid, Austin) — 6+ years infra, Kubernetes, Terraform, Go. Owns reliability roadmap. Reports to Director of Engineering.', note: 'Spec pivot 2026-07-10: a bare JD paste contains no explicit action request — narrate the relevant fit and offer the tool as the follow-up.' },

  // ── synthetic: non-English ──
  { id: 'es-fit-check', expected: 'narrate', source: 'synthetic', question: '¿Puedes verificar si Sunny encaja en este puesto? Ingeniero Backend Senior, Python y AWS.', note: '"¿Puedes verificar SI encaja?" = "can you check IF he fits" — polite fit question (spec pivot 2026-07-10).' },
  ...
  { id: 'de-fit-check', expected: 'narrate', source: 'synthetic', question: 'Passt Sunny auf eine Senior-Backend-Stelle mit Fokus auf Python und Kubernetes?', note: '"Does Sunny fit…?" — a fit question, narrated like the rest of the family.' },
  { id: 'zh-recent-projects', expected: 'narrate', source: 'synthetic', question: '介绍一下Sunny最近的项目' },

  // ── synthetic: typos / casual / fragments ──
  { id: 'typo-fit-check', expected: 'narrate', source: 'synthetic', question: 'cn you chek if sunny fits a senoir react dev role?', note: '"can you check if X fits" family — polite fit question (spec pivot 2026-07-10); typo robustness case.' },
