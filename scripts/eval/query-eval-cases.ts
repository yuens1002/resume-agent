...
// The legacy `action_intent` eval category (#174) was removed in #195: the
// 112-case route eval (scripts/eval/route-cases.ts, run via `npm run
// eval:route`) is now the single source of truth for routing coverage.
// Keeping both here risked stale duplicate labels — several of this
// category's labels were already stale after the #195 narrate-first spec
// pivot. See scripts/eval/route-cases.ts and src/lib/route-classifier.ts.
