...
- **`/mcp`** — **private**, authenticated. Exposes Open Brain and job pipeline tools, including the [structured pipeline feed](job-pipeline-feed.md) and [materialized application-evidence snapshots](application-evidence-snapshot.md). Source: `src/routes/mcp.ts`.
