...
| AC-03 | D1 | /backend-architect | Clock-driven due work | Executable SQL | Due and overdue applied-or-later applications appear without a new mutation; future, draft, and terminal entries do not | PASS (source scope) | PASS — independent verification and review; see review.md | |
...
| AC-05 | D2 | /backend-architect | Typed contract | Adapter tests | Valid source payload, including draft stage snapshots, passes schema; malformed, unknown-stage, error, and exception payloads return refusal and no next cursor; input constraints enforced | PASS (source scope) | PASS — independent verification and review; see review.md | |
