...
**Follow-up archaeology (2026-07-29).** The ceiling never held. Row counts for `type: reference` at each point the value was set:

| Date | Commit | Ceiling set to | `reference` rows then |
|---|---|---|---|
| 2026-06-01 | #131 (endpoint ships) | 300 | **1,999** |
| 2026-06-02 | #132 | 1,000 | **2,158** |
| 2026-07-29 | — | 1,000 | **3,485** |

So the justifying comment — "the ceiling sits above the largest single thought type" — was false on the day it was written, by more than 2×, and the original 300 was off by nearly 7×. This is not decay; the invariant never existed. `?type=reference&topic=…` has been silently incomplete since the endpoint shipped.

The important property is that the failure is **invisible from the response**: a capped result is shaped exactly like a complete one, and `total` is itself computed after the cap, so it reports the floor rather than the truth.
