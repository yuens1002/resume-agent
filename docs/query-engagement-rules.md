...
**Regression (#182) — the opposite direction:** #180's fix over-corrected and made the tool require what read like a pasted, block-formatted job posting before firing, so prose-style fit-check requests ("Can you check if Sunny would be a good fit for this role? Staff Engineer, Platform team — deep experience with distributed systems and Kubernetes required") consistently failed to call the tool and asked for "the full job description" instead — even though a title, team, and requirement were already given. The fix is a **two-part gate, checked independently**: (i) an explicit fit-check/tailor/match request must be present — without it, no amount of role detail qualifies (this is what keeps #180 fixed: "What projects has Sunny built?" has role-adjacent words but no fit-check verb, so it still doesn't call the tool); and (ii) at least one loose role-identifying signal (title, team, company — named or anonymized like "a Series B startup" — a skill, or a scope/responsibility phrase) is enough on its own, no pasted block or named company required. Only ask for more detail when (i) is present but (ii) is entirely absent (e.g. "would this role be a good match?" alone). Collapsing (i) and (ii) into a single signal count reopened #180 during development — the two checks must stay independent.
