...
## Action intent — a dedicated classifier pre-pass, not a tool (#195)

Routing used to be an AI SDK tool, `open_match_tool`, exposed to the model alongside the normal JSON response (#174). That design asked the model to both answer the question AND decide whether to open the job-match/résumé-tailoring flow in the same generation call — the routing rule competed with the rest of this document's ~15k chars of instructions and misfired on capability questions that had no fit-check language at all (#194).

**#195 replaces it with a dedicated classifier pre-pass.** Before `queryProfile` ever calls `generateText`, it calls `classifyRoute(question)` (`src/lib/route-classifier.ts`) — a separate `generateObject` call with forced enum output (`'narrate' | 'open_match_tool'`), question-only input, and its own compact rule text (`ROUTE_CLASSIFIER_RULE`). The two calls run in parallel with retrieval via `Promise.all`, so the pre-pass adds ~0 wall-clock.

- **Route `open_match_tool`:** `queryProfile` short-circuits — no `generateText` call at all. It returns a deterministic response (`answer: "Opening the job-fit tool now."`, `action_intent: { tool: "open_match_tool" }`) so the shape stays backward-compatible with `resume-agent-web`.
- **Route `narrate`:** `generateText` runs exactly as any other answer, with no tool passed at all — there is nothing left for a routing rule to misfire against during generation — and `response.action_intent` is always `null`.
- **Classifier error:** falls back to `narrate` (the safe default).

This document's rules above no longer say anything about `open_match_tool` — there is no `RULE_ACTION_INTENT` composed into this prompt anymore. The routing spec lives entirely in `ROUTE_CLASSIFIER_RULE`, and its regression coverage lives in the route-classifier golden set (`scripts/eval/route-cases.ts`, run via `npm run eval:route`), not in this document's eval cases.

Surfaces that cannot act on `action_intent` (MCP/API callers, via `src/routes/public-mcp.ts`) never forward the raw short-circuit answer — they rewrite it into a narrated pointer at the interactive flow instead, so a caller with no UI never receives a dead-end "Opening the tool now." with nothing to open. Full spec/rationale: GitHub issues #174 (original tool-calling design), #194 (the bug this replaces), #195 (this architecture), #183 (the surface-capability gap).
